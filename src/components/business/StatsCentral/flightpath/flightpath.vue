          ...
          <div class="process-order-li-center">
            ...
            <div class="foutersgdhgdfe">
              <img src="../../../../assets/飞机.png" alt="">
                <el-button type="primary" class="ckshihi" @click="flightpath(item.seq,item.uav_id)">查看实时轨迹</el-button>
            </div>
          </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    flightpath: function(seq, uav_id) {
      console.log(seq, uav_id);
      const data = {
        seq: seq,
        uav_id: uav_id
      };
      this.$store.commit("setfiightpath", data);
      location.href = "#/business/history";
    }
  },
  mounted() {
    this.getuseid();

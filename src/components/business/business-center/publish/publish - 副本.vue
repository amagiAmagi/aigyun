...
<div class="publish" >
  ...
  <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
    ...
    <el-form-item label="作业时间" required class="required">
      <el-col :span="6">
         ...
         <div class="dklist" ref="dklist">
           <div class="information" v-for="(item,index) in this.plotList" :key="index" @click="showcar(item)">
             ...
             <div class="information-right">
               ...
               <span>{{item.farmland_ares}}亩</span><br><span>{{item.crops_name}}</span>
             </div>
           </div>
         </div>
         <span class="el-icon-arrow-left" v-show="right" @click="rights"></span>
         <span class="el-icon-arrow-right" v-show="length>4" @click="left"></span>
       </div>
...
export default {
  ...
  methods: {
      ...
      this.$http
        .post(
          api.apihost + "TaskManager",
          {
            ...
            task_other_type: "",
            crops_type: this.cropList.type,
            crops_name: this.cropList.name,
            task_time: Math.round(this.ruleForm.date.getTime() / 1000),
            task_prov: "",
            task_city: "",
            task_district: "",
            ...
            task_addr: this.this.ruleForm.site,
            demand_type: 1
          },
          {
            headers: {
    ...
    left: function() {
      this.num += 1;
      this.right = true;
      this.$refs.dklist.className = "dklist lefts";
      if (this.num == 2 && this.num <= this.index) {
        this.$refs.dklist.className = "dklist leftss";
      }
      if (this.num == 3 && this.num <= this.index) {
        this.length = 4;
        this.$refs.dklist.className = "dklist leftsss";
      }
    ...
    rights: function() {
      if (this.right) {
        ...
        if (this.num == 1 && this.num <= this.index) {
          this.$refs.dklist.className = "dklist lefts";
        }
        if (this.num == 0 && this.num <= this.index) {
          this.$refs.dklist.className = "dklist";
          this.right = false;
        }
      }
    }
